...
El proyecto ya tiene inicializado el CLI de Supabase (`supabase/` en la raíz) para desarrollo local con Docker, con el **esquema relacional inicial ya migrado** (tablas de perfiles, reservaciones y los tres servicios adicionales — ver sección 3 y el detalle en `CLAUDE.md`). **El login ya usa Supabase Auth de verdad, y las tablas de usuarios y reservaciones del panel admin ya son CRUD real** (ver sección 6); el resto de la UI (fotos, amenidades, servicios) sigue leyendo de `lib/mock-data.ts` (ver sección 8).
...
lib/
  mock-data.ts          → Datos de prueba que siguen mockeados: fotos, amenidades, servicios, precios, reservaciones del panel admin, disponibilidad de spa/comida/vinos (los usuarios del panel admin ya NO están aquí — ver profiles en Supabase, sección 6)
...
⚠️ Los **usuarios** del panel de administración ya **no** viven en `lib/mock-data.ts` (el array `mockUsers` se eliminó) — son la tabla real `profiles` de Supabase. Para agregar uno nuevo de prueba, usa el botón "Crear usuario" en `/admin` (ver sección 6) en vez de editar este archivo.

⚠️ Las **reservaciones** tampoco viven ya en `lib/mock-data.ts` (el array `mockReservations` se eliminó) — son la tabla real `reservations` de Supabase. Para agregar una de prueba, usa el botón "Crear reservación" en `/admin/reservations` (ver sección 6).
...
- **Sección "Usuarios invitados"** (**CRUD real completo, ya no mock** — Fase 1: leer/crear/editar, Fase 2: eliminar + creación directa con contraseña temporal; ver también `CLAUDE.md`): [components/UsersTable.tsx](components/UsersTable.tsx) recibe las filas reales de `profiles` (fetch hecho en `app/admin/page.tsx` con el cliente de servidor estándar, junto con el `id` del propio admin vía `supabase.auth.getUser()`) y ya no guarda ninguna copia local en `useState` — lo que se ve en pantalla es exactamente la prop que le llegó del servidor, y se refresca solo cuando una Server Action de [app/admin/actions.ts](app/admin/actions.ts) llama a `revalidatePath("/admin")`.
  ...
  - Botón **"Editar"** de cada fila abre el mismo modal interactivo de siempre (fondo `bg-black/40 backdrop-blur-sm`, panel `bg-white rounded-2xl shadow-2xl`, animación `@keyframes modal-in` en [app/globals.css](app/globals.css), cierra con "Cancelar"/clic fuera/Escape), que **solo edita rol y estado** (ya no nombre/correo, porque la Server Action `updateUser` no soporta esos campos) — el correo se muestra de solo lectura como referencia. Llama a la Server Action `updateUser(userId, { role, status })`.
...
- **Políticas RLS pendientes**: la migración inicial (`supabase/migrations/20260901072551_init_schema.sql`) dejó Row Level Security deshabilitado en todas las tablas (ver sección 3.1) — hoy `profiles` y `reservations` son las únicas tablas en uso real desde la app (login/middleware, y el CRUD de usuarios y de reservaciones en `/admin`, ver sección 6), pero cualquier cliente con la `anon key` (ej. desde la consola del navegador, sin pasar por la app) todavía podría leer o escribir cualquier fila de cualquier tabla, `profiles`/`reservations` incluidas — ambos CRUD solo están protegidos a nivel de aplicación (cada Server Action valida `role === "admin"` antes de tocar la tabla, vía el guard compartido `requireAdmin()`), no a nivel de base de datos. Falta esa migración de políticas antes de exponer más tablas a componentes reales, y sigue pendiente también para `profiles`/`reservations` a pesar de que ya se escribe en ellas.
...
- **Panel de administración** ([app/admin/page.tsx](app/admin/page.tsx) y [app/admin/reservations/page.tsx](app/admin/reservations/page.tsx)): ~~la tabla de usuarios era de solo lectura sobre datos mockeados~~ / ~~la tabla de reservaciones era de solo lectura sobre mockReservations~~ — **ya resuelto para ambas**: `UsersTable` lee, crea, edita y elimina filas reales de `profiles`/`auth.users` vía Server Actions (`createUser`/`updateUser`/`deleteUser` en `app/admin/actions.ts`), y `ReservationsTable` hace lo mismo sobre `reservations` vía `getReservations`/`createReservation`/`updateReservation`/`deleteReservation` en `app/admin/reservations/actions.ts` (ver sección 6).
  - `deleteUser` ya bloquea que un admin se elimine a sí mismo (guard en la propia Server Action + botón deshabilitado en la UI). **`updateUser` no tiene ese mismo guard todavía**: un admin sí puede quitarse su propio rol de admin o suspenderse (`status: "invitado"`) desde el modal de edición, y quedaría sin acceso a `/admin` en su siguiente navegación — recuperable solo interviniendo la base de datos a mano, igual que la auto-eliminación que sí está bloqueada.
  ...
  - El chequeo de solapamiento de fechas de `createReservation`/`updateReservation` solo compara contra reservaciones ya `confirmada` — no existe todavía el concepto de "bloque de mantenimiento" (fechas bloqueadas sin huésped real, ej. para limpieza o reparaciones). La arquitectura ya deja la puerta abierta para eso sin cambiar el chequeo de solapamiento en sí (ver CLAUDE.md, "CRUD de reservaciones", para el detalle de cómo).
