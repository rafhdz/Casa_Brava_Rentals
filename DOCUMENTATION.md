...
│   ├── weather.ts                Clima en vivo de Open-Meteo para el hero de la landing
│   ├── parras-events.ts          Catálogo de 12 meses (evento/ciclo del mes) para el hero de la landing
...
| Widget "Parras de la Fuente, hoy" del hero — primera fila: clima en vivo de Open-Meteo | [components/ParrasStatusWidget.tsx](components/ParrasStatusWidget.tsx), [lib/weather.ts](lib/weather.ts) |
| Widget "Parras de la Fuente, hoy" del hero — segunda fila: evento/ciclo del mes en curso | [lib/parras-events.ts](lib/parras-events.ts) (`PARRAS_MONTHLY_EVENTS`) — editar solo este archivo, ver mapeo de íconos en `ParrasStatusWidget.tsx` |
