...
- **Botón "Volver"**: [components/BackButton.tsx](components/BackButton.tsx) es un componente cliente minimalista (`useRouter().back()` de `next/navigation`) montado arriba del contenido en `app/servicios/spa/page.tsx`, `app/servicios/comida/page.tsx`, `app/servicios/vinos/page.tsx`, `app/reservar/page.tsx`, `app/carrito/page.tsx` y `app/perfil/page.tsx`, para que el usuario nunca quede "atrapado" en esas vistas de flujo. Al ser Server Components envueltos en `<ProtectedRoute>`, importar `<BackButton />` (Client Component) no obliga a convertir la página entera en cliente — `app/perfil/page.tsx` ya era Client Component desde antes, así que ahí simplemente se importa igual.
