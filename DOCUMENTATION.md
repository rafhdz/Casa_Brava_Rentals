...
app/
  ...
  admin/page.tsx        → Dashboard de administración (usuarios reales + reservaciones mock) — ruta protegida por middleware.ts, requiere role === "admin"
...
Para agregar una nueva reservación de prueba, basta con agregar un objeto más al array `mockReservations` en `lib/mock-data.ts` — la tabla de reservaciones del panel de administración se actualiza automáticamente.
...
- **Sección "Reservaciones"**: tabla con los datos de `mockReservations` usando [components/ReservationsTable.tsx](components/ReservationsTable.tsx), filtrando en la propia página (`app/admin/page.tsx`) para no mostrar las reservaciones con estado `pasada`. A diferencia de "Usuarios invitados", esta tabla **sigue sin conectarse** a la tabla real `reservations` — queda para una fase futura.
- **Ahora sí tiene guard de ruta real**: `middleware.ts` exige sesión con `role === "admin"` en `profiles` antes de dejar pasar a `/admin` (ver arriba) — a diferencia del prototipo anterior, ya no es accesible por URL directa sin cumplir ambas condiciones. Las Server Actions de `app/admin/actions.ts` repiten esa misma verificación de rol por su cuenta (no confían en que solo se llamen desde una página ya protegida), porque una Server Action es un endpoint invocable directamente, no solo un handler de UI.
...
- **Políticas RLS pendientes**: la migración inicial (`supabase/migrations/20260901072551_init_schema.sql`) dejó Row Level Security deshabilitado en todas las tablas (ver sección 3.1) — hoy `profiles` es la única tabla en uso real desde la app (login/middleware, y desde esta fase también el CRUD de usuarios en `/admin`, ver sección 6), pero cualquier cliente con la `anon key` (ej. desde la consola del navegador, sin pasar por la app) todavía podría leer o escribir cualquier fila de cualquier tabla, `profiles` incluida — el CRUD de `/admin` solo está protegido a nivel de aplicación (cada Server Action valida `role === "admin"` antes de tocar la tabla), no a nivel de base de datos. Falta esa migración de políticas antes de exponer más tablas a componentes reales, y sigue pendiente también para `profiles` a pesar de que ya se escribe en ella.
...
- **Panel de administración** ([app/admin/page.tsx](app/admin/page.tsx)): ~~la tabla de usuarios era de solo lectura sobre datos mockeados~~ — **ya resuelto para usuarios**: `UsersTable` lee, crea, edita y elimina filas reales de `profiles`/`auth.users` vía Server Actions (`createUser`/`updateUser`/`deleteUser` en `app/admin/actions.ts`, ver sección 6). Sigue pendiente la tabla de **reservaciones**, todavía de solo lectura sobre `mockReservations` — falta conectarla a la tabla real `reservations`.
