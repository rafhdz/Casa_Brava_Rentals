import type { AmenityCategory } from "@/lib/mock-data";
  ...
  return (
    <div className="flex flex-col gap-8">
      {categories.map((category) => (
        <div key={category.id}>
          <h3 className="text-sm font-semibold text-neutral-900">{category.category}</h3>
          <ul className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {category.items.map((amenity) => (
              ...
              >
                ...
                <img src={amenity.url} alt="" aria-hidden className="h-5 w-5 shrink-0 opacity-80" />
