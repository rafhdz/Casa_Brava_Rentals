...
| [types.ts](lib/api/types.ts) | **Todos los contratos de la API en TypeScript.** Escrito a mano contra los serializers del backend; no hay generador. Reemplaza al antiguo archivo de tipos generado desde el esquema de la base. Exporta además tres listas en tiempo de ejecución derivadas de esos enums: `ESTADOS_ACTIVOS`, `ESTADOS_CONSOLIDADOS` (`confirmada` + `finalizada`: el universo de las métricas de `/admin`) y `MEAL_TYPES` (los tres tiempos de comida, que alimentan el select del catálogo de menús). |
...
| [server.ts](lib/api/server.ts) | `serverFetch`, `serverFetchAll`, `publicFetchAll`, `getSessionUser`, `toActionError` y `nullOnApiError`. Resuelve la sesión, refresca si hace falta y reintenta ante un 401. `nullOnApiError` es el `.catch()` estándar de un fetch en un Server Component: un `ApiError` se vuelve `null` (aviso de "backend caído") y cualquier otra excepción —el `redirect()` de sesión expirada— se vuelve a lanzar. **No usar `.catch(() => null)` a secas.** |
...
1. **Los decimales viajan como `string`, no como `number`.**
   ...
   **Nunca operar aritméticamente sobre ellos directamente** — `"4500.00" * 2` no es lo que parece. Convertir antes con `toNumber()` de [lib/format.ts](lib/format.ts), o formatear con `formatMoney()` (con separador de miles: `$1,284,500.00`). La conversión se hace **en la página** (Server Component), para que los componentes de presentación sigan recibiendo números limpios.
   Para **sumar o multiplicar dinero**, trabajar en centavos enteros con `toCents()` (redondeo mitad-hacia-afuera, el mismo `ROUND_HALF_UP` del backend) y dividir entre 100 solo al formatear: `0.1 + 0.2` no da `0.3` en binario, y el error se acumula al sumar cientos de reservaciones. Para **mandar** un monto a un `DecimalField`, usar `toDecimalString()` (string con exactamente 2 decimales): un float como `2025.0000000000002` lo rechaza DRF con un 400.
...
**Un borrado puede fallar por diseño.** Las FK de los cuatro catálogos son `on_delete=PROTECT` desde las reservaciones y sus bookings, para que borrar una fila del catálogo no reescriba lo ya cobrado — también si la reservación está en soft delete, porque el historial sigue ahí. Ese intento responde **409** con su mensaje en español: `apiFetch` lo aplana con `extractErrorMessage`, `toActionError` lo devuelve como `{ error }`, `CatalogTable` lo muestra con `toast.error` y el modal **se queda abierto** (retorna antes de `setRowToDelete(null)`) para que el admin dé marcha atrás. El contrato 409 de los cuatro catálogos está cubierto por pruebas del backend. La traducción de `ProtectedError` a 409 se agregó en el backend (`casabrava_core/exceptions.py`, registrado como `EXCEPTION_HANDLER`): sin ella salía como 500 con traceback HTML, y `extractErrorMessage` habría metido ese HTML entero dentro de un toast. **El límite real es la API**, no el copy del modal.
...
- **No operar aritméticamente sobre los campos decimales sin `toNumber()`** — llegan como string (ver "Capa de acceso a la API"). Y no sumar dinero en pesos con decimales: centavos con `toCents()`.
