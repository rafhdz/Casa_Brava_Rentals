...
- `/admin` ([app/admin/page.tsx](app/admin/page.tsx)) **sigue sin protección de ruta real**: es accesible directamente por URL. `/perfil` ([app/perfil/page.tsx](app/perfil/page.tsx)) sí redirige a `/login` cuando `useAuth().user` es `null` (una vez que `isLoading` es `false`), pero es una redirección en el cliente, no un guard de servidor/middleware. No agregar un guard real (middleware, cookies de sesión firmadas) hasta que se pida explícitamente — cuando se pida, debe basarse en Supabase Auth, no en una solución mock adicional.
